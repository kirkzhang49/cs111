...
void* Sub_routine(void* n)
{
  int receive_length;
  char buf2[BUFSIZE];
  memset(buf2,0,sizeof(buf2));
	receive_length = SSL_read(ssl, buf2, sizeof(buf2));
	if(strstr(buf2, "new_rate: ") != NULL)
	{
	    sscanf(buf2, "Heart rate of patient %*s is %*f new_rate: %d", &rate);
	    rate = rate;
	    printf("New rate %d received from server.\n", rate);
      //fprintf(log,"New rate %d received from server.\n", rate);
      receive_length = SSL_read(ssl, buf2, sizeof(buf2));
	}
	printf("Received message '%s' from server.\n\n", buf2);
  fprintf(log,"Received message '%s' from server.\n", buf2);
  fflush(log);
}
...
int main(int args, char *argv[])
{
  ...
  fprintf(log, "Sending message '%s' to server...\n", buf);
  fflush(log);
 
	SSL_write(ssl, buf, strlen(buf));
  
  
  pthread_create(&pid,NULL,Sub_routine,NULL);
